...
def xyz2ply_w_sp(xyz, args):
    ...
    if args.verbose:
        with args.Lock:
            msg = f'Worker process started for xyz -> ply: {xyz}'
            print(msg)
            write_to_log(msg, args.log_file)
...
if __name__ == '__main__':
    ...
    args.Lock = m.Lock()
    [tile_data(sp, args) for sp in np.sort(args.ScanPos)] # run in series
    # Pool.starmap(tile_data, [(sp, args) for sp in np.sort(args.ScanPos)]) # run in parallel
